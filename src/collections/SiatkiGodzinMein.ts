...
export const SiatkiGodzinMein: CollectionConfig = {
  ...
  admin: {
    useAsTitle: 'id',
    defaultColumns: ['przedmiot', 'typ_szkoly', 'klasa', 'godziny_w_cyklu', 'obowiazkowe', 'updatedAt'],
  },
  access: {
    read: () => true,
    create: () => true,
  ...
  fields: [
    ...
    },
    {
      ...
      label: 'Obowiązuje do',
      admin: {
        description: 'Data, do której obowiązują wymagania (pozostaw puste, jeśli nadal obowiązują)',
      },
    },
    {
      name: 'uwagi',
    ...
    beforeValidate: [
      ({ data }) => {
        ...
        if (data.godziny_tygodniowo_min && data.godziny_tygodniowo_max) {
          if (data.godziny_tygodniowo_min > data.godziny_tygodniowo_max) {
            throw new Error('Minimalne godziny tygodniowo nie mogą być większe od maksymalnych');
          }
        }
        return data;
      },
